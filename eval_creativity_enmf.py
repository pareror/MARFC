# =============================================================================
# EVALUATION SCRIPT FOR CREATIVITY SCORE-BASED RECOMMENDATION RERANKING - ENMF
# =============================================================================
# This script evaluates trained ENMF (Efficient Neural Matrix Factorization) models
# and applies a reranking based on a creativity score formula that combines relevance,
# novelty, and unexpectedness. Includes fixes for ENMF-specific reshape and history filtering.
#
...
RESULTS_FILE = 'benchmark_creativity_enmf.csv'
LOG_FILE = 'eval_creativity_enmf.log'
...
def custom_load_model(model_file):
    """Load ENMF checkpoint and configure for evaluation.
    
    Uses RecBole's load_data_and_model utility to load a pre-trained ENMF model
    from checkpoint file and initializes dataset and data splits. Configures
    batch sizes, metrics, and device for evaluation.
    
    Args:
        model_file: path to checkpoint .pth file containing model state and config
    
    Returns:
        tuple: (config, model, dataset, train_data, valid_data, test_data)
            - config: RecBole configuration object
            - model: loaded and initialized ENMF model on GPU
...
def get_vectors_for_yan(model):
    """Extract user and item embeddings from ENMF model.
    
    Args:
        model: trained ENMF model
    
    Returns:
        tuple: (user_embeddings, item_embeddings) or (None, None) if not available
    """
...
# =============================================================================
# RECOMMENDATION GENERATION WITH SCORES - ENMF FIX
# =============================================================================
def generate_recs_with_scores(model, test_data, topk, user_history):
    """Generate top-K recommendations with relevance scores for all test users.
    
    Includes fixes for ENMF-specific reshape issues and history filtering.
    Processes each batch in the test set and returns top-K items per user with scores.
    
    Args:
        model: trained ENMF model
        test_data: test data loader
        topk: number of recommendations per user
        user_history: dict mapping user_id -> tensor of seen items
    ...
    with torch.no_grad():
        for batch in tqdm(test_data, desc="Generating Lists with Scores", leave=False):
            ...
            # CRITICAL FIX: Safe reshape for ENMF
            ...
            # History filtering to remove seen items
...
def evaluate_checkpoint(model_path):
    ...
    try:
        ...
        # =================================================================
        # STEP 1: Run RecBole evaluation (baseline metrics)
        # =================================================================
        ...
        user_recs_with_scores = generate_recs_with_scores(model, test_data, max_candidates, user_history)
        ...
        for num_candidates in CANDIDATE_KS:
            ...
            # ============================================================
            # Custom serendipity/unexpectedness metrics
            for k in TOPKS:
                # Original metrics (only for K=max candidates to avoid duplicates)
...
def main():
    ...
    try:
        print("=" * 80)
        print("CREATIVITY SCORE RERANKING EVALUATION - ENMF")
